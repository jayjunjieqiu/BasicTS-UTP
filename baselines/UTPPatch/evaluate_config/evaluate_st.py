...
BASELINE_PREFIX = 'baselines/UTPExp2/evaluate_config/'

DATASET_LIST = ['PEMS03', 'PEMS04', 'PEMS07', 'PEMS08', 'PEMS-BAY', 'METR-LA']
MODEL_LIST = ['utp_base.py']
CHECKPOINT_PATH_List = [
    '/data/junjieqiu/BasicTS-0.5.8/checkpoints/UTP/BLAST_100000/utp_base/UTP_best_val_loss.pt',
]
CONTEXT_LENGTH_LIST = [12, 96]
            ...
            for context_length in CONTEXT_LENGTH_LIST:
                ...
                for line in ouput.split('\n'):
                    if 'Result <test>:' in line:
                        ...
                        with open('utp_st_evaluation_results.csv', 'a') as f_csv:
                            f_csv.write(f'{dataset},{model},{context_length},{prediction_length},{mae},{mse}\n')
                        break

    # write logs to file
    with open('utp_st_evaluation_logs.txt', 'w') as f:
        for log in logs:
            f.write(log)
            f.write('\n' + '='*80 + '\n')

    # write eval_result to file
    with open('utp_st_evaluation_results.txt', 'w') as f:
        for key, value in eval_result.items():
            f.write(f'{key}: MAE={value["MAE"]}, MSE={value["MSE"]}\n')
        
    with open('utp_st_evaluation_results.json', 'w') as f:
        import json
