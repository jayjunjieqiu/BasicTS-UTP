...
BASELINE_PREFIX = 'baselines/UTPExp/evaluate_config/'

DATASET_LIST = ['ETTh1', 'ETTh2', 'ETTm1', 'ETTm2', 'Weather']
MODEL_LIST = ['utp_base.py']
CHECKPOINT_PATH_List = [
    '/data/junjieqiu/BasicTS-0.5.8/checkpoints/UTP/BLAST_100000/b2f6df3412d391c9fda6f9a94fa3be03/UTP_best_val_loss.pt',
]
                ...
                for prediction_length in PREDICTION_LENGTH_LIST:
                    ...
                    for line in ouput.split('\n'):
                        if 'Result <test>:' in line:
                            ...
                            with open('utp_evaluation_results.csv', 'a') as f_csv:
                                f_csv.write(f'{dataset},{model},{context_length},{prediction_length},{mae},{mse}\n')
                            break

    # write logs to file
    with open('utp_evaluation_logs.txt', 'w') as f:
        for log in logs:
            f.write(log)
            f.write('\n' + '='*80 + '\n')

    # write eval_result to file
    with open('utp_evaluation_results.txt', 'w') as f:
        for key, value in eval_result.items():
            f.write(f'{key}: MAE={value["MAE"]}, MSE={value["MSE"]}\n')
    
    with open('utp_evaluation_results.json', 'w') as f:
        import json
