...
class UTPModel(nn.Module):
    def __init__(self, config: UTPModelConfig):
        ...
        aux_cfg = (self.config.byot or {}).get('AUX_HEAD_LAYERS', []) if (self.config.byot is not None) else []
        aux_cfg = sorted(set([i for i in aux_cfg if isinstance(i, int) and 0 <= i < config.num_layers]))
        self.aux_layers = aux_cfg
        self.aux_heads = nn.ModuleDict({str(i): PointPredictHead(config.embed_dim, config.mlp_hidden_dim, num_quantiles) for i in self.aux_layers})
    ...
    def forward_byot(self, x: torch.Tensor, x_mask: torch.Tensor, context_query_split_index: int):
        x = self.ts_encoder(x, x_mask, context_query_split_index)
        if self.config.use_reg_token:
            bs, T_total, num_feat, embed_dim = x.shape
            reg = self.reg_token.expand(bs, 1, num_feat, embed_dim)
        ...
        x_t = x[:, :, 0, :]
        if self.config.use_reg_token:
            x_t = torch.cat([x_t[:, :context_query_split_index, :], x_t[:, context_query_split_index+1:, :]], dim=1)
        teacher = self.predict_head(x_t)
        students = {}
        for idx in self.aux_layers:
            hx = hiddens[idx][:, :, 0, :]
            if self.config.use_reg_token:
                hx = torch.cat([hx[:, :context_query_split_index, :], hx[:, context_query_split_index+1:, :]], dim=1)
            head = self.aux_heads[str(idx)]
            students[idx] = head(hx)
        teacher = teacher[:, context_query_split_index:, :]
        students = {k: v[:, context_query_split_index:, :] for k, v in students.items()}
